...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（7062 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 7062 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）与 `apps/web/src/run-cache.js`（输入指纹/缓存），页面主流程仍留在 `app.js`。
...
### P4：模块化工程结构 — 2/10 第一阶段完成

路线建议拆分 10 个模块，当前已完成第一阶段低风险拆分：纯领域数据和缓存指纹逻辑已从 `app.js` 移出。

| 建议模块 | 当前状态 | 当前行数估算 |
|----------|----------|-------------|
| i18n.js | ❌ 未拆分 | ~500 行（`i18n` 对象 + `applyLanguage`） |
...
**变化**：已完成第一刀低风险拆分，`app.js` 从 8151 行降至 7062 行；`domain-data.js` 和 `run-cache.js` 可单独做语法检查，并已纳入 smoke test。

**影响**：最大工程债已开始拆解，但 `app.js` 仍超过 7000 行，报告拼装、图谱渲染、虚拟委员会、PDF 导出和 i18n 仍需继续模块化。

**建议加强**：
1. 下一步拆 `i18n.js`，把文案字典与语言切换从页面编排中移出。
2. 再拆 `graph.js` 和 `virtual-panel.js`，优先降低最高频修改区域的维护成本。
...
| P4 模块化 | 0/10 | 2/10 | 已拆出 domain-data 与 run-cache 第一阶段模块 |
| P0 测试 | 0/3 | 0/3 | 无变化 |
...
2. P4 模块化（已完成第一阶段拆分，但 app.js 仍超过 7000 行）
...
2. **app.js 仍超过 7000 行**（P4 第一阶段完成）— 最大工程债已开始拆解，但仍需继续拆分

...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（5852 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/src/i18n.js`、`apps/web/src/virtual-panel.js`、`apps/web/src/evidence-graph.js`、`apps/web/src/graph-view.js`、`apps/web/src/model-client.js`、`apps/web/src/reports-view.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`scripts/virtual_panel_test.js`、`scripts/evidence_graph_test.js`、`scripts/graph_view_test.js`、`scripts/model_client_test.js`、`scripts/reports_view_test.js`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 5852 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）、`apps/web/src/run-cache.js`（输入指纹/缓存）、`apps/web/src/i18n.js`（中英文文案/报告阶段）、`apps/web/src/virtual-panel.js`（委员会纯模型层）、`apps/web/src/evidence-graph.js`（图谱纯模型层）、`apps/web/src/graph-view.js`（图谱渲染/筛选/详情/报告跳转）、`apps/web/src/model-client.js`（真实模型请求/SSE/错误协议）与 `apps/web/src/reports-view.js`（报告渲染/流式进度/摘要/评分表），报告视图层已不再由 `app.js` 直接维护。
...
### P4：模块化工程结构 — 8/10 第七阶段完成

路线建议拆分核心职责模块，当前已完成第七阶段渐进拆分：纯领域数据、缓存指纹逻辑、国际化文案、虚拟委员会模型、EvidenceGraph 模型、图谱视图逻辑、模型网络客户端和报告视图层已从 `app.js` 移出。
...
| reports-view.js | ✅ 第七阶段完成 | 报告 Markdown 转 HTML、流式进度、生成失败态、决策摘要和面试官评分表已进入 `apps/web/src/reports-view.js`；报告内容生成仍在 `app.js` |
...
**变化**：已完成七阶段渐进拆分，`app.js` 从 8151 行降至 5852 行；八个独立模块可单独做语法检查，委员会、图谱模型、图谱视图、模型客户端和报告视图均有 Node 行为测试并纳入 smoke test。

**影响**：最大工程债持续拆解，`app.js` 已降到 6000 行以内；报告内容生成、委员会 UI 和 PDF 导出仍需继续模块化。

**建议加强**：
1. 下一步拆 `panel-view.js`，隔离虚拟委员会聊天渲染、轮次筛选和 trace 入口。
2. 再拆 `pdf.js`，隔离 PDF HTML、截图、字体和打印窗口逻辑。
...
2. P4 模块化（已完成第七阶段拆分，app.js 已降到 6000 行以内）
...
4. **P4 继续拆核心模块**（panel-view / pdf / report-builders）— 为后续开发降本
...
2. **app.js 仍有 5852 行**（P4 第七阶段完成）— 最大工程债已持续拆解，但委员会 UI、PDF 和报告内容生成仍需继续拆分

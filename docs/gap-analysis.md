...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（6334 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/src/i18n.js`、`apps/web/src/virtual-panel.js`、`apps/web/src/evidence-graph.js`、`apps/web/src/graph-view.js`、`apps/web/src/model-client.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`scripts/virtual_panel_test.js`、`scripts/evidence_graph_test.js`、`scripts/graph_view_test.js`、`scripts/model_client_test.js`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 6334 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）、`apps/web/src/run-cache.js`（输入指纹/缓存）、`apps/web/src/i18n.js`（中英文文案/报告阶段）、`apps/web/src/virtual-panel.js`（委员会纯模型层）、`apps/web/src/evidence-graph.js`（图谱纯模型层）、`apps/web/src/graph-view.js`（图谱渲染/筛选/详情/报告跳转）与 `apps/web/src/model-client.js`（真实模型请求/SSE/错误协议），模型网络层已不再由 `app.js` 直接维护。
...
### P4：模块化工程结构 — 7/10 第六阶段完成

路线建议拆分核心职责模块，当前已完成第六阶段渐进拆分：纯领域数据、缓存指纹逻辑、国际化文案、虚拟委员会模型、EvidenceGraph 模型、图谱视图逻辑和模型网络客户端已从 `app.js` 移出。
...
**变化**：已完成六阶段渐进拆分，`app.js` 从 8151 行降至 6334 行；七个独立模块可单独做语法检查，委员会、图谱模型、图谱视图和模型客户端均有 Node 行为测试并纳入 smoke test。

**影响**：最大工程债持续拆解，`app.js` 已降到 6400 行以内；报告拼装、委员会 UI 和 PDF 导出仍需继续模块化。

**建议加强**：
1. 下一步拆 `reports.js`，隔离报告拼装、决策摘要、评分表和流式预览。
2. 再拆 `panel-view.js`，隔离虚拟委员会聊天渲染、轮次筛选和 trace 入口。
...
2. P4 模块化（已完成第六阶段拆分，app.js 已降到 6400 行以内）
...
4. **P4 继续拆核心模块**（reports / panel-view / pdf）— 为后续开发降本
...
2. **app.js 仍有 6334 行**（P4 第六阶段完成）— 最大工程债已持续拆解，但报告、委员会 UI 和 PDF 仍需继续拆分

...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（7126 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/src/i18n.js`、`apps/web/src/virtual-panel.js`、`apps/web/src/evidence-graph.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`scripts/virtual_panel_test.js`、`scripts/evidence_graph_test.js`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 7126 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）、`apps/web/src/run-cache.js`（输入指纹/缓存）、`apps/web/src/i18n.js`（中英文文案/报告阶段）、`apps/web/src/virtual-panel.js`（委员会纯模型层）与 `apps/web/src/evidence-graph.js`（图谱纯模型层），页面主流程和图谱渲染仍留在 `app.js`。
...
### P4：模块化工程结构 — 5/10 第四阶段完成

路线建议拆分 10 个模块，当前已完成第四阶段渐进拆分：纯领域数据、缓存指纹逻辑、国际化文案、虚拟委员会模型和 EvidenceGraph 模型已从 `app.js` 移出。
...
| evidence-graph.js / graph | ✅ 纯模型层完成 | `buildEvidenceGraph`、`reportAnchorForNodeType`、`detectEvidenceGraphGaps` 已进入独立模块；渲染、筛选、详情弹窗和 trace 仍在 `app.js` |
...
**变化**：已完成四阶段渐进拆分，`app.js` 从 8151 行降至 7126 行；五个独立模块可单独做语法检查，委员会与图谱纯模型均有 Node 行为测试并纳入 smoke test。

**影响**：最大工程债持续拆解，但 `app.js` 仍超过 7000 行；报告拼装、图谱构建与渲染、委员会 UI 和 PDF 导出仍需继续模块化。

**建议加强**：
1. 下一步拆 `graph-view.js`，把图谱渲染、筛选、详情弹窗和报告跳转移出页面编排。
2. 再拆 `model-client.js`，隔离真实模型请求、流式解析、代理地址和错误处理。
...
2. P4 模块化（已完成第四阶段拆分，app.js 仍超过 7000 行）
...
4. **P4 继续拆核心模块**（graph-view / model-client / reports）— 为后续开发降本
...
2. **app.js 仍有 7126 行**（P4 第四阶段完成）— 最大工程债已持续拆解，但仍需继续拆分

...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（7381 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/src/i18n.js`、`apps/web/src/virtual-panel.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`scripts/virtual_panel_test.js`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 7381 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）、`apps/web/src/run-cache.js`（输入指纹/缓存）、`apps/web/src/i18n.js`（中英文文案/报告阶段）与 `apps/web/src/virtual-panel.js`（委员会纯模型层），页面主流程和委员会渲染仍留在 `app.js`。
...
**影响**：虚拟委员会纯模型已有独立 Node 行为测试，但 UI 交互、缓存命中路径和报告渲染仍主要依赖静态文本检查，尚未形成端到端自动化防护网。
...
### P4：模块化工程结构 — 4/10 第三阶段完成

路线建议拆分 10 个模块，当前已完成第三阶段渐进拆分：纯领域数据、缓存指纹逻辑、国际化文案和虚拟委员会纯模型层已从 `app.js` 移出。
...
| graph.js | ❌ 未拆分 | ~700 行（`buildEvidenceGraph` + 渲染 + 筛选 + trace） |
...
**变化**：已完成三阶段渐进拆分，`app.js` 从 8151 行降至 7381 行；四个独立模块可单独做语法检查，委员会纯模型层新增 Node 行为测试并纳入 smoke test。

**影响**：最大工程债持续拆解，但 `app.js` 仍超过 7000 行；报告拼装、图谱构建与渲染、委员会 UI 和 PDF 导出仍需继续模块化。

**建议加强**：
1. 下一步先拆 `graph.js` 的纯模型层，把 EvidenceGraph 节点、关系、缺口检测和引用构建移出页面编排。
2. 再拆虚拟委员会渲染与追溯导航，减少 `app.js` 中最高耦合的 DOM 逻辑。
...
| P4 模块化 | 0/10 | 4/10 | 已拆出 domain-data、run-cache、i18n 与 virtual-panel 纯模型层 |
| P0 测试 | 0/3 | 0/3 | 三项路线要求未完成；新增虚拟委员会纯模型行为测试 |
| 总完成度 | 14/30 项 | **19/30 项** | +5 项 |
| 完成率 | 47% | **63%** | +16% |
...
2. P4 模块化（已完成第三阶段拆分，app.js 仍超过 7000 行）
...
4. **P4 继续拆核心模块**（virtual-panel / graph / reports）— 为后续开发降本
...
**当前最大的三个短板**：
1. **端到端与缓存回归测试仍缺失**（P0 三项未完成）— 目前只有局部纯模型行为测试
2. **app.js 仍有 7381 行**（P4 第三阶段完成）— 最大工程债已持续拆解，但仍需继续拆分

...
- 输入质量预检：量化结果、个人贡献边界、失败复盘、JD 职责和上下文完整度。
- 图谱节点级解释：节点详情展示“为什么值得关注”。
- 反馈前后对比：摘要区和反馈侧栏展示问题、风险、虚拟角色权重变化。
- 模型连接测试：生成前验证 Base URL、API Key 和模型名。
- 本机缓存明细：展示最近缓存条目并支持单条删除。
- 复用导出：Markdown 导出、ATS 交接卡复制、Notion 摘要复制。
- 候选人 30 分钟准备计划和面试官本轮面试流程卡。
...
2. 继续拆分 `app.js`。
3. 将 smoke test 机械拆分为 UI/schema/report/cache 四类。
4. 增加真实模型错误态浏览器级回归。
5. 优化大图谱密度、排序和折叠。

团队协作、云端报告和真实样本评测集暂不推进。

...
本报告对照 `开发路线.md` 中定义的 P0-P5 优先级清单，逐项验证 `apps/web/app.js`（6723 行）、`apps/web/src/domain-data.js`、`apps/web/src/run-cache.js`、`apps/web/src/i18n.js`、`apps/web/styles.css`、`apps/web/index.html`、`scripts/smoke_test.py`、`schemas/` 目录 5 个 JSON Schema 的实际实现状态。

**本次评估相比上次（13:42 版本）的变化**：
- `app.js` 已从 8151 行拆分到 6723 行，新增 `apps/web/src/domain-data.js`（岗位/样例/常量）、`apps/web/src/run-cache.js`（输入指纹/缓存）与 `apps/web/src/i18n.js`（中英文文案/报告阶段），页面主流程仍留在 `app.js`。
...
**影响**：所有验证都是静态文本匹配（正则 / 关键词），无法覆盖运行时行为。任何 UI 交互回归、缓存命中路径、报告渲染逻辑变更都没有自动化防护网。
...
### P4：模块化工程结构 — 3/10 第二阶段完成

路线建议拆分 10 个模块，当前已完成第二阶段低风险拆分：纯领域数据、缓存指纹逻辑和国际化文案已从 `app.js` 移出。
...
| virtual-panel.js | ❌ 未拆分 | ~800 行（VirtualPanel + 讨论 + 主持人 + BasisTrace） |

**变化**：已完成两阶段低风险拆分，`app.js` 从 8151 行降至 6723 行；`domain-data.js`、`run-cache.js` 和 `i18n.js` 可单独做语法检查，并已纳入 smoke test。

**影响**：最大工程债持续拆解，`app.js` 已降至 7000 行以内；报告拼装、图谱渲染、虚拟委员会和 PDF 导出仍需继续模块化。

**建议加强**：
1. 下一步先拆 `virtual-panel.js` 的纯模型层，把委员会成员、三轮讨论与主持人总结构建逻辑移出页面编排。
2. 再拆 `graph.js`，优先降低最高频修改区域的维护成本。
...
| P4 模块化 | 0/10 | 3/10 | 已拆出 domain-data、run-cache 与 i18n 三个模块 |
| P0 测试 | 0/3 | 0/3 | 无变化 |
| 总完成度 | 14/30 项 | **18/30 项** | +4 项 |
| 完成率 | 47% | **60%** | +13% |
...
2. P4 模块化（已完成第二阶段拆分，app.js 已降至 7000 行以内）
...
2. **app.js 仍有 6723 行**（P4 第二阶段完成）— 最大工程债已持续拆解，但仍需继续拆分

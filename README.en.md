...
7. Frontend modularization phase 5 is complete: `apps/web/src/domain-data.js` owns role/sample/constants, `apps/web/src/run-cache.js` owns input fingerprints and run caching, `apps/web/src/i18n.js` owns localized copy and report-progress stages, `apps/web/src/virtual-panel.js` owns the virtual-panel model, `apps/web/src/evidence-graph.js` owns the graph model, and `apps/web/src/graph-view.js` owns graph rendering, filtering, trace-detail modals, and report navigation; `apps/web/app.js` still owns page orchestration, reports, panel rendering, model requests, and PDF flow for the next split.

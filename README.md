...
用户选择目标岗位（产品经理、开发人员、技术支持人员、销售人员），并输入候选人简历、岗位 JD 和可选上下文后，系统先判断候选人的项目经历是否支撑岗位核心职责，再生成候选人准备报告、面试官追问报告、证据链、风险点、Offer 推演状态和可导出 PDF 报告。
...
把“目标岗位 + 简历 + JD”转成一份可追问、可复盘、可下载的面试准备报告，并生成候选人追问题库。
...
1. 候选人简历。
2. 岗位 JD。
3. 目标岗位：产品经理、开发人员、技术支持人员、销售人员。
4. 公司 / 面试上下文。
5. 候选人阶段，例如初筛、业务一面、业务终面、Offer 前。
6. 目标职级。
7. Offer / 谈薪约束。
8. 面试官视角，例如 HR、业务负责人、项目推进、谈薪顾问、决策层压力官。
...
4. 根据目标岗位 RoleProfile，对齐岗位职责与项目经历，判断候选人的项目能力是否支持 JD。
5. 输出“匹配进入 / 条件性进入（转岗适配） / 不匹配不推进”三类闸口结果。
6. 如果缺少项目证据，输出待验证或不匹配点。
7. 如果简历与 JD 表面匹配，继续生成反包装追问。

当前实现中，匹配判断仍以规则 + 关键词 + 模型生成为主，不是严格的结构化评分系统，但已经完成结论证据化、证据可信度分级、条件性进入、能力迁移话术、目标岗位 RoleProfile 和 EvidenceGraph 最小结构的前端展示。
...
1. 完成纯静态 Web 页面。
2. 支持本地 Mock Demo。
3. 支持简历、JD、目标岗位、公司上下文、候选人阶段、目标职级、Offer 约束输入。
4. 支持目标岗位下拉选择：产品经理、开发人员、技术支持人员、销售人员。
5. 支持面试官视角选择。
6. 支持分块生成报告。
7. 支持候选人报告和面试官报告拆分。
8. 支持 PDF 报告导出。
9. 支持候选人、面试官、Offer 推演三类 PDF 导出。
10. 支持人工反馈写入当前报告。
11. 支持中文 / English 双语切换。
12. 支持 GitHub Pages 部署。
...
16. 动态校准指令。
17. 决策层压力官固定压力测试。
18. EvidenceGraph 关系图谱展示、筛选、节点详情和报告段落跳转。
19. 证据缺口提示：有结论但缺证据、有风险但无追问、JD 要求未覆盖。
20. FeedbackDistillation 可视化：升级、降权、删除、保留及影响说明。
21. OfferSimulationRun 结构化状态：运行阶段、历史、三场景对比和状态回填提示。

### 结构化运行状态

当前 Web MVP 已经从“只生成报告”推进到“报告 + 可回填运行状态”：

1. `EvaluationRun`：记录输入快照、目标岗位、报告输出、证据、反馈和版本信息。
2. `OfferSimulationRun`：记录 Base / Optimistic / Conservative 三种 Offer 路径、阶段状态和下一轮策略输入。
3. `EvidenceGraph`：把 JD、简历、证据、问题、风险、反馈和 Offer 信号连接成最小关系图谱。
4. `FeedbackDistillation`：把人工反馈归类为升级、降权、删除、保留，并生成影响说明和 Skill 更新建议。
5. `SkillDefinition`：已整理固定面试官视角示例，后续会继续演进为可选择、可组合、可版本化的 Skill Registry。
...
2. 还没有团队协作、项目空间或历史记录。
3. 还没有报告云端保存。
4. 还没有可编辑、可版本化的岗位模板库；当前只有内置四类 RoleProfile。
5. 还没有公司模板库。
6. 还没有候选人多轮面试记录串联。
7. 还没有真正持久化的面试后反馈学习闭环；当前只有单次运行内的 FeedbackDistillation 展示。
8. 还没有批量简历评估。
9. 还没有跨候选人的结构化评分看板。
10. 还没有权限管理。
...
4. EvidenceGraph 已有最小展示和缺口检测，但缺少持久化、可编辑和跨运行复盘能力。
5. 已有四类内置 RoleProfile，但缺少岗位能力矩阵的配置化、版本化和自动选择能力。
...
7. FeedbackItem：人工反馈。
8. OfferSimulationRun：Offer 推演状态、阶段历史、场景对比和状态回填输入。
9. EvidenceGraph：JD、简历、证据、问题、风险、反馈和 Offer 信号之间的关系。
10. SkillDefinition：面试官视角、岗位能力框架和反馈沉淀建议。

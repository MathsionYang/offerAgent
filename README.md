...
7. 已完成第二阶段前端模块化：`apps/web/src/domain-data.js` 承载岗位/样例/常量，`apps/web/src/run-cache.js` 承载输入指纹与缓存，`apps/web/src/i18n.js` 承载中英文文案与报告进度阶段；`apps/web/app.js` 仍保留页面编排、报告、图谱、委员会和 PDF 主流程，后续继续拆分。
